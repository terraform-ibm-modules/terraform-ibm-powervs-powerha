...
| <a name="input_powerha_glvm_volume_group_list"></a> [powerha\_glvm\_volume\_group\_list](#input\_powerha\_glvm\_volume\_group\_list) | List of parameters for GLVM volume group - Individual PowerHA GLVM volume group configuration. Based on the powerha\_glvm\_volume\_group count, you can provide all the GLVM volume group configurations like name, type, size, and tier. The default configuration will be taken if details are not provided. | <pre>list(object({<br>    name = string<br>    type = string<br>    size = number<br>    tier = string<br>  }))</pre> | n/a | yes |
...
| <a name="input_site1_subnet_list"></a> [site1\_subnet\_list](#input\_site1\_subnet\_list) | IBM Cloud Power Virtual Server subnet configuration details for site1 like name and CIDR. Ensure no duplicate subnet names or CIDRs. A maximum of 14 subnets are allowed. | <pre>list(object({<br>    name = string<br>    cidr = string<br>  }))</pre> | n/a | yes |
...
| <a name="input_site2_subnet_list"></a> [site2\_subnet\_list](#input\_site2\_subnet\_list) | IBM Cloud Power Virtual Server subnet configuration details for site2 like name and CIDR. Ensure no duplicate subnet names or CIDRs. A maximum of 14 subnets are allowed. | <pre>list(object({<br>    name = string<br>    cidr = string<br>  }))</pre> | n/a | yes |
